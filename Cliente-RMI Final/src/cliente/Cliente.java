...
public class Cliente extends JFrame implements ActionListener, Runnable{
    ...
    public static String IPServer = "192.168.0.118";                 //Dirección IP del servidor, la cual podría utilizarse por defecto el localhost
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if( evt.getSource()==Cliente.botonReady ){
            Cliente.Ready=true;
            Cliente.botonReady.setEnabled(false);}
             ///////Esto es lo nuevo/////
            ...
            if (conexion.iniciarRegistro(IPServer, Puerto, usuarioRefRemoto) && conexion.iniciarRegistro(IPServer, Puerto, tableroRefRemota) && conexion.iniciarRegistro(IPServer, Puerto, chatRefRemota)) {
                ...
                while (opcion != 5) {
                        ...
                        if (ingreso) {
                            ...
                            Cliente.confirmarNombre.setEnabled(false);
                        } 
                        
                        while(!ingreso)
                        {
                            Thread.sleep(1000);
                            ingreso = usuarioRemoto.ingresarUsuario(Cliente.esteUsuario);
                            
                            if(ingreso){Cliente.confirmarNombre.setEnabled(false);
                                break;}
                    ...
                    while(CantidadUsuarios!=CantidadListos){
                       ...
                       {   
                           ...
                           Cliente.validadorUsuario=0;
                               ...
                               Cliente.esteUsuario=Cliente.auxiliarUsuario;
                               Cliente.validadorUsuario=0;
                           }
                       }
